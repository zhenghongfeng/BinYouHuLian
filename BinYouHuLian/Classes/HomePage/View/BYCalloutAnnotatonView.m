...
@interface BYCalloutAnnotatonView () {
...
@implementation BYCalloutAnnotatonView
...
+ (instancetype)calloutViewWithMapView:(MKMapView *)mapView{
    ...
    BYCalloutAnnotatonView *calloutView=(BYCalloutAnnotatonView *)[mapView dequeueReusableAnnotationViewWithIdentifier:calloutKey];
    if (!calloutView) {
        calloutView=[[BYCalloutAnnotatonView alloc]init];
    }
    return calloutView;
}

#pragma mark 当给大头针视图设置大头针模型时可以在此处根据模型设置视图内容
- (void)setAnnotation:(BYCalloutAnnotation *)annotation
{
