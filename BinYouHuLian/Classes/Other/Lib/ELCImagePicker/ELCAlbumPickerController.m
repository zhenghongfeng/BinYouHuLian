...
//                    [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Access Denied", nil) message:errorMessage delegate:nil cancelButtonTitle:NSLocalizedString(@"Ok", nil) otherButtonTitles:nil] show];
                    [[[UIAlertView alloc] initWithTitle:@"提示" message:@"请在设备的‘设置-隐私-照片’中允许访问相册" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil] show];
...
//- (void)dealloc
//{
//    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"SetNotLogin" object:nil];
//}
//- (void)setNotLoginNotification
//{
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(goToLogin) name:@"SetNotLogin" object:nil];
//}
//- (void)goToLogin
//{
//    
//    //    [Singleton goToViewWithClassName:@"KKLoginViewController" nav:nil animate:YES];
//    
//    [self exitFromAccount];
//    [[NSNotificationCenter defaultCenter] postNotificationName:@"outLine" object:nil];//被挤掉线时重置头像和昵称的通知
//    KKLoginViewController *login = [[KKLoginViewController alloc] init];
//    login.loginType = PresentLogin;
...
//{
//    //    KKUserInfoEntity *userInfoEntity = [KKUserInfoEntity findFirstByAttribute:@"account" withValue:[COMMONMODULE curAccount]];
//    if (Singleton.userInfo.account) {
//        //        [userInfoEntity deleteEntity];
//        //        [[NSManagedObjectContext defaultContext] saveOnlySelfAndWait];
//        COMMONMODULE.isLogin = NO;
//        Singleton.Is_Logon = NO;
//        Singleton.userInfo.account = nil;
//        Singleton.userInfo.userId = nil;
//        Singleton.userInfo.accesstoken = nil;
//        Singleton.userInfo.refreshtoken = nil;
//        Singleton.userInfo.avatarUrl = nil;
//        Singleton.userInfo.nickName = nil;
//        
//    }else{
//        //        MYInfo(@"当前未登录");
//    }
...
{
	[self.tableView reloadData];
	[self.navigationItem setTitle:NSLocalizedString(@"选取相册", nil)];
}
