...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"请选择不超过%d张", nil), self.maximumImagesCount];
//        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"一次选择的照片数不要超过5张", nil), self.maximumImagesCount];
